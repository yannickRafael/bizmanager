...
/// All cattle sales across batches — 2 tabs: Cattle Sales + Milk Sales.
class CattleSalesScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return DefaultTabController(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: const Text('🐄 Vendas de Bovinos'),
          leading: IconButton(
...
class _AddSaleFab extends StatelessWidget {
        ...
        child: Column(
          ...
          children: [
            ...
            FilledButton.icon(
              onPressed: () {
                Navigator.pop(ctx);
                _showAddCattleSaleDialog(context);
              },
              icon: const Icon(Icons.pets),
              label: const Text('Venda de Gado'),
            ),
  ...
  void _showAddCattleSaleDialog(BuildContext context) {
    final cattle = context.read<CattleProvider>();
    final clients = context.read<ClientProvider>();
    final batches = cattle.batches.where((b) => b.status == BatchStatus.active).toList();

    if (batches.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Não há manadas activas.')),
      );
      return;
    }
    ...
    String? clientId;
    CattleSaleType saleType = CattleSaleType.live;
    ...
    showDialog(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setState) => AlertDialog(
          title: const Text('Nova Venda de Gado'),
          content: Form(
            key: formKey,
              ...
              child: Column(
                ...
                children: [
                  DropdownButtonFormField<String>(
                    value: batchId,
                    decoration: const InputDecoration(labelText: 'Manada'),
                  ...
                  const SizedBox(height: 12),
                  DropdownButtonFormField<CattleSaleType>(
                    value: saleType,
                    decoration: const InputDecoration(labelText: 'Tipo de Venda'),
                    items: const [
                      DropdownMenuItem(value: CattleSaleType.live, child: Text('Vivo')),
                      DropdownMenuItem(value: CattleSaleType.slaughtered, child: Text('Abatido')),
                    ],
            ...
            FilledButton(
              onPressed: () {
                if (formKey.currentState!.validate()) {
                  ...
                  cattle.addCattleSale(CattleSale(
                    id: const Uuid().v4(),
                    batchId: batchId,
                    clientId: clientId,
                    saleType: saleType,
...
class _CattleSalesTab extends StatelessWidget {
  const _CattleSalesTab();
  ...
  Widget build(BuildContext context) {
    ...
    final sales = cattle.cattleSales;

    if (sales.isEmpty) {
      return const EmptyStateWidget(
        icon: Icons.shopping_cart_outlined,
        message: 'Nenhuma venda de gado registada.',
      );
    }
    ...
    return ListView.builder(
      ...
      itemBuilder: (ctx, i) {
        ...
        final batch = cattle.getBatchById(s.batchId);
          ...
          child: ListTile(
            ...
            trailing: IconButton(
              ...
              onPressed: () async {
                ...
                if (confirmed) cattle.deleteCattleSale(s.id);
              },
            ),
          ),
        );
...
class _MilkSalesTab extends StatelessWidget {
  const _MilkSalesTab();
        ...
        return Card(
          child: ListTile(
            ...
            trailing: IconButton(
              ...
              onPressed: () async {
                final confirmed = await showDeleteConfirmation(context, itemName: 'esta venda');
                if (confirmed) cattle.deleteMilkSale(s.id);
              },
            ),
          ),
        );
